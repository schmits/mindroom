...
manager, or GitHub publishing client.  It can create short-lived repository
workspaces from already-approved local materialization roots, expose file and
diff operations inside those workspaces, and produce explicit handoff manifests
for a separate execution sandbox.
"""
...
class RepoWorkspaceTools(Toolkit):
    ...
    * execution requests are represented as handoff descriptors for a separate
    ...
    def write_file(self, workspace_id: str, path: str, content: str, confirm_write: bool = False) -> str:
        """Write a UTF-8 text file inside the workspace repo. Requires confirmation."""
        if not confirm_write:
    ...
    def delete_file(self, workspace_id: str, path: str, confirm_write: bool = False) -> str:
        """Delete a file inside the workspace repo. Requires confirmation."""
        if not confirm_write:
    ...
    def apply_patch(self, workspace_id: str, patch: str, confirm_write: bool = False) -> str:
        """Apply a unified patch within the workspace repo. Requires confirmation."""
        if not confirm_write:
    ...
    def _load_workspace(self, workspace_id: str) -> tuple[dict[str, Any], Path] | str:
        ...
        validation_error = self._validate_workspace_metadata(metadata, workspace_dir)
        if validation_error:
    ...
    def _validate_workspace_metadata(self, metadata: dict[str, Any], workspace_dir: Path) -> str | None:
        return _validate_workspace_metadata(metadata, workspace_dir, configured_source_roots=self.allowed_source_roots)
...
def _format_completed_process(prefix: str, result: subprocess.CompletedProcess[str]) -> str:
    output = result.stderr or result.stdout or "unknown error"
    return f"Error: {prefix}: {_truncate_output(output)}"
...
def _truncate_output(value: str) -> str:
    ...
    if len(encoded) <= _MAX_COMMAND_OUTPUT_BYTES:
        return value
    return encoded[:_MAX_COMMAND_OUTPUT_BYTES].decode("utf-8", errors="replace") + "\n[truncated]"